...
また、python3コマンド単体で実行すると、対話モードに入る。対話モードについては __-i__ オプションで説明する。**対話モードは、`Ctrl+D` で抜けられる。**
実行している最中に`Ctrl+C`を押すと、プログラムを強制終了できる。
...
- **-i または、python3単体で実行** 
  ...
  -iオプションまたはpython3単体で実行すると、対話モードに入る。対話モードに入ると、式の演算や変数の定義を操作などを行うことができる。対話モードは `Ctrl+D` で抜けられる。
